...
interface SemesterDao {
    @Query("SELECT s.id AS id, s.name AS name, SUM(c.credit_hours) AS total_credit_hours, COUNT(c.id) AS num_courses, s.sgpa AS sgpa FROM semesters s LEFT JOIN courses c ON s.id = c.semester_id GROUP BY s.id ORDER BY s.created_at")
